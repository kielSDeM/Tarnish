...
FILE1=~/tarnish/src/linux/virt/kali.qcow2
FILE2=~/tarnish/src/linux/virt/qemu_iso/kali-linux-2022.1-installer-amd64.iso
#creates kali virtual disk if it does not exist
if [ ! -f "$FILE1" ]
then
    echo "Going to create a kali virt-disk & download kali iso"
    qemu-img create -f raw kali.qcow2 30G 
else
    echo "files exist"
fi
...
then
    qemu-system-x86_64 -drive file=~/tarnish/src/linux/virt/kali.qcow2,format=raw -boot d \
    -cdrom ~/tarnish/src/linux/virt/qemu_iso/kali-linux-2022.1-installer-amd64.iso  -m 4096M -cpu host \
    -smp 2 -enable-kvm 
    ...
    -netdev user,id=mynet0,net=192.168.76.0/24,dhcpstart=192.168.76.5 \
    -device e1000,netdev=mynet0,mac=30:5F:33:D1:44:ee 
fi 